...
class Puzzle:
        ...
        # So you can pass a puzzle that has a plant missing in the middle, even though it is unreachable.
        # Complete verification is a bit more involved, since you could be a dragon standing in the middle, trying to box yourself in.
                if len(self.buf) != 5:
                        raise Except("Incorrect buffer length")
                try:
                ...
                if total % 5 != 0:
                        raise Exception("Your starting position must have the number of plants divisible by 5. Check both the board and the buffer.")
                #print(total)
                return True
        ...
        #the_list: [3, 4] -- list of recipes that it begins. 0, 1, or 2 members (no 3 recipes start from the same plant)
        ...
        # Verifies that solution moves achieve the promised recipe list.
                ...
                for m in solution.moves:
                        if not m in puzzle.get_available_moves():
...
class Solution:
        ...
        def add_move(self, move, recipe_achieved = None):
                ...
                if not recipe_achieved is None:
...
class Solution_Set:
        ...
        # Also, if solutions_achieved is provided, then we remove all solutions dominated by solutions_achieved
...
# In this function we use the fact that the board size = buf size * 5, and every recipe is length 5.
...
def theo_find_filler_moves(puzzle, solution_so_far, recipe_list):
        ...
        for i in range(5): # loop over all recipes
                for j in range(5):
                        remainder[j] -= recipe_list[i] * g_recipes_totals[i, j] # can I vectorize this when I remove numpy?
...
# This returns a Solution_Set
        ...
        for s in theo_solutions:
                ...
                # If you add a constant to every set in a set of optimal solutions, the solutions stay optimal
                new_sol.recipes_achieved = list(map(add, new_sol.recipes_achieved, s)) # add from operator package
                #print("sol so far + s = ", new_sol.recipes_achieved)
                for old_s in solutions_achieved:
                        if theo_is_dominated_by(new_sol.recipes_achieved, old_s.recipes_achieved): # no point adding it
...
# First version. No hashing. Just a few heuristics
        ...
        if not remaining_theo_possible: # no solutions
                ...
                return
        theo_solutions_plus = [list(map(add, ts, solution_so_far.recipes_achieved)) for ts in remaining_theo_possible]
        ...
        if puzzle.is_every_square_reachable():
                ...
                more_solutions = theo_solve_puzzle(puzzle.copy(), solution_so_far.copy(), solutions_achieved)
...
# 3) moves that unlock more moves. 
                ...
                if the_list:
                        for r in the_list:
                                for m in range(len(moves)):
                                        if g_recipes[r, num_non_zero] == puzzle.board[moves[m]]: # board[m[0], m[1]]: 
        ...
        new_moves = [x for _, x in sorted(zip(move_weights, moves), key = lambda pair: pair[0], reverse = True)]
        return new_moves
